...
class NFSDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        ...
        anno_path = f"{self.base_path}/{sequence_info['name'][4:]}/30/{sequence_info['name'][4:]}.txt"
        
        # ground_truth_rect = load_text(str(anno_path), delimiter=' ', dtype=np.float64)
        # ground_truth_rect = load_text(str(anno_path), delimiter=',', dtype=np.float64)
        ground_truth_rect_ori = np.loadtxt(str(anno_path), delimiter=( ' '), dtype=np.float64, usecols=[1, 2, 3, 4])
        # ground_truth_rect[:,2] = ground_truth_rect[:,2] - ground_truth_rect[:,0]
        # ground_truth_rect[:,3] = ground_truth_rect[:,3] - ground_truth_rect[:,1]
        ground_truth_rect = np.zeros((ground_truth_rect_ori.shape[0], 4)) #u,v,u2,v2 -> x,y,w,h
        ground_truth_rect[:, 0] = ground_truth_rect_ori[:, 0]
        ground_truth_rect[:, 1] = ground_truth_rect_ori[:, 1]
        ground_truth_rect[:, 2] = ground_truth_rect_ori[:, 2] - ground_truth_rect_ori[:, 0]
        ground_truth_rect[:, 3] = ground_truth_rect_ori[:, 3] - ground_truth_rect_ori[:, 1]
        # pdb.set_trace()
        # evaluation in 30 FPS
