...
def enlarge_bboxes_xywh_torch(bboxes, scale=1.2):
    """
    bboxes: Tensor of shape (N, 4) with [x, y, w, h] format
    scale:  float, enlargement factor
    """
    ...
    enlarged = torch.stack([new_x, new_y, w, h], dim=1)
    return enlarged
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            if os.path.isfile(results_path):
                ...
                if 'rvcot_nfs' in results_path:
                    scale = 1.025
                    pred_bb = enlarge_bboxes_xywh_torch(pred_bb, scale=scale)
                    print(f"Enlarge the bbox by {scale} on rvcot_nfs", results_path)
                pred_bb[pred_bb<0]=0.0                
            else:
                if skip_missing_seq:
